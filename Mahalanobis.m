...
for i = 1:17
    means(i, :) = mean( train( (train_pos(1,i):train_pos(2,i)), 2:191) );
end

labels = zeros(17, 3197);

% Covariance calculation and inverse
covclass = cov(train(:, 2:191));
invcov = inv(covclass);
...
%for i = 1
    ...
    %for j = 1:145*145
        ...
        mins = zeros(1,17);
        for k = 1:17
            d = (test(j, 2:191) - means(k,:))*invcov*(test(j, 2:191) - means(k,:))';
            %d = (data(j, 2:191) - means(k,:))*invcov*(data(j, 2:191) - means(k,:))';
            mins(1, k) = d;
        end
